    ...
    return dispatch => {
        ...
        fetch("http://localhost:8081/upload", {
            ...
            .catch(e => {
                ...
                reader.onload = () =>{
                    dispatch(uploadFile(reader.result, file.name, keywords, file.type));
        ...
        if (query.title !== null && query.title.length > 0) {
            if (query.file !== null) {
                ...
                if (filename.match(/\.(jpeg|jpg|gif|png)$/) !== null) {
                    ...
                    let mimetype = "*";
                    if (filenameDeconstructed.length > 1) {
                        mimetype = "image/" + filenameDeconstructed[filenameDeconstructed.length - 1];
                    }
