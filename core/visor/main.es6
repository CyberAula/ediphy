...
export default {
    ...
    exportsDoc: function(state, callback, options = { forcePageBreak: true, slidesPerPage: 1, slidesWithComments: false, optionName: "defaultOption", drawBorder: false }) {
                ...
                setTimeout(
                    () => {
                        ...
                        if(last) {
                            ...
                            for (let l = 0; l < numPages; l++) {
                                content += document.getElementById('pageContainer_' + l).innerHTML;
                            }
                            content += "</body></html>";
                            window.download(content, "Ediphy.docx", "application/msword");
                            if(!isSafari) {
                                deletePageContainers('pageToPrint');
                            }
                            callback();
