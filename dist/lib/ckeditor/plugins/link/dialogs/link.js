...
( function() {
	CKEDITOR.dialog.add( 'link', function( editor ) {
        ...
        var pluginOptions, pluginAliases, pluginFunctions;
		...
		var linkTypeChanged = function() {
				var dialog = this.getDialog(),
					partIds = [ 'urlOptions', 'anchorOptions', 'emailOptions', 'pluginOptions'],
        ...
        var setupPluginSelector = function (data){
            switch(this.id){
                case 'pluginType':
                    this.clear();
                    pluginOptions.forEach(function (opt){
                        this.add(opt);
                        if(opt === data.pluginType){
                            this.setValue(opt);
                        }
                    }.bind(this));
                    break;
                case 'pluginAlias':
                    this.clear();
                    if(pluginAliases[this.getDialog().getContentElement('info', 'pluginType').getValue()]) {
                        pluginAliases[this.getDialog().getContentElement('info', 'pluginType').getValue()].forEach(function (opt) {
                            this.add(opt);
                            if (opt === data.pluginAlias) {
                                this.setValue(opt);
                            }
                        }.bind(this));
                    }
                ...
                case 'pluginFunction':
                    this.clear();
                    if(pluginAliases[this.getDialog().getContentElement('info', 'pluginType').getValue()]) {
                        pluginFunctions[this.getDialog().getContentElement('info', 'pluginType').getValue()].forEach(function (opt) {
                            this.add(opt);
                            if (opt === data.pluginFunction) {
                                this.setValue(opt);
                            }
                        }.bind(this));
                    }
        ...
        var pluginTypeChanged = function(){
            var pluginType = this.getDialog().getContentElement('info', 'pluginType');
            var pluginAlias = this.getDialog().getContentElement('info', 'pluginAlias');
            var pluginFunction = this.getDialog().getContentElement('info', 'pluginFunction');

            pluginAlias.clear();
            pluginFunction.clear();
            pluginAliases[pluginType.getValue()].forEach(function (opt){
                pluginAlias.add(opt);
            }.bind(this));
            pluginFunctions[pluginType.getValue()].forEach(function (opt){
                pluginFunction.add(opt);
            }.bind(this));
        }
		...
		return {
			...
			contents: [ {
				...
				},
                {
                    ...
                    children: [{
                        ...
                        label: 'Tipo',
                        items: [],
                        onChange: pluginTypeChanged,
                        setup: setupPluginSelector,
                        commit: function( data ) {
                            data.pluginType = this.getValue();
                        }
                    }, {
                        id: 'pluginAlias',
                        type: 'select',
                        ...
                        setup: setupPluginSelector,
                        commit: function( data ) {
                            data.pluginAlias = this.getValue();
                        }
                    }, {
                        id: 'pluginFunction',
                        type: 'select',
                        ...
                        setup: setupPluginSelector,
                        commit: function( data ) {
                            data.pluginFunction = this.getValue();
                        }
                    }]
                },
				{
			...
			onShow: function() {
                ...
                if(element) {
                    data.type = element.getAttribute('data-cke-type') || 'url';
                    data.pluginType = element.getAttribute('data-cke-pluginType') || '';
                    data.pluginAlias = element.getAttribute('data-cke-pluginAlias') || '';
                    data.pluginFunction = element.getAttribute('data-cke-pluginFunction') || '';
                }
			...
			onOk: function() {
                ...
                attributes.set['data-cke-type'] = data.type;
                ...
                if(data.pluginType !== "" &&
                    ...
                    attributes.removed.push('data-cke-pluginFunction');
                    if(data.type === 'plugin'){
                        attributes.set['data-cke-saved-href'] = "";
                        attributes.removed.push('href');
                    }
                }
