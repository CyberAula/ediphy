...
const Folder = ({ name, collapsed, index, onToggleCollapse }) => {
    ...
    return (
        <div className={classNames(style.body, style.folder) + " folder"}>
            <button className={style.toggleCollapseHandle + " folder"} onClick={handleClick}>
                <i className={"material-icons folder"}>folder_open</i>
            </button>
            {name}
        </div>
    );
...
const File = ({ name, collapsed }) => (
    <div className={'file ' + classNames(style.body, style.file, { [style.collapsed]: collapsed })}>
