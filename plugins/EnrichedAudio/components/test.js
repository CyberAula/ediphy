...
!(function(a, b) {
    typeof define === "function" && define.amd ? define("wavesurfer", [], function() {
        return a.WaveSurfer = b();
    }) : typeof exports === "object" ? module.exports = b() : a.WaveSurfer = b();
})(this, function() {
    ...
    var a = {
        defaultParams: {
            ...
            if (this.params = a.util.extend({}, this.defaultParams, b), this.container = typeof b.container === "string" ? document.querySelector(this.params.container) : this.params.container, !this.container) {
                throw new Error("Container element not found");
            }
            if (this.params.mediaContainer == null ? this.mediaContainer = this.container : typeof this.params.mediaContainer === "string" ? this.mediaContainer = document.querySelector(this.params.mediaContainer) : this.mediaContainer = this.params.mediaContainer, !this.mediaContainer) {
            ...
            let b = this;
            this.backend && this.backend.destroy(), this.params.backend == "AudioElement" && (this.params.backend = "MediaElement"), this.params.backend != "WebAudio" || a.WebAudio.supportsWebAudio() || (this.params.backend = "MediaElement"), this.backend = Object.create(a[this.params.backend]), this.backend.init(this.params), this.backend.on("finish", function() {
                b.fireEvent("finish");
            ...
            let b = this.getCurrentTime() || 0, c = this.getDuration() || 1;
            ...
            let b = this.backend.isPaused();
            b || this.backend.pause();
            let c = this.params.scrollParent;
            ...
            let a = Math.round(this.getDuration() * this.params.minPxPerSec * this.params.pixelRatio),
                b = this.drawer.getWidth(), c = a, d = this.drawer.getScrollX(), e = Math.min(d + b, c);
            if (this.params.fillParent && (!this.params.scrollParent || a < b) && (c = b, d = 0, e = c), this.params.partialRender) {
                for (let f = this.peakCache.addRangeToPeakCache(c, d, e), g = 0; g < f.length; g++) {
            ...
            let d = a;
            if (typeof a === "string") {
                this.backend.load(d, this.mediaContainer, b, c);
            } else {
                let e = a;
            ...
            let d = this, e = a.util.ajax({ url: b, responseType: "arraybuffer" });
            ...
            let d = this.backend.getPeaks(a, b), e = [].map.call(d, function(a) {
    ...
    return a.create = function(b) {
        ...
        extend: function(a) {
            ...
            let c = Object.create(a.Observer), d = new XMLHttpRequest(), e = !1;
            return d.open(b.method || "GET", b.url, !0), d.responseType = b.responseType || "json", d.addEventListener("progress", function(a) {
                ...
                e || c.fireEvent("progress", a), c.fireEvent("load", a), d.status == 200 || d.status == 206 ? c.fireEvent("success", d.response, a) : c.fireEvent("error", a);
        ...
        on: function(a, b) {
            ...
            if (this.handlers) {
                let b = this.handlers[a], c = Array.prototype.slice.call(arguments, 1);
        ...
        scriptBufferSize: 256, PLAYING_STATE: 0, PAUSED_STATE: 1, FINISHED_STATE: 2, supportsWebAudio: function() {
            ...
            if (!this.mergedPeaks || a != 2 * this.mergedPeaks.length - 1 + 2) {
                ...
                for (let b = this.buffer ? this.buffer.numberOfChannels : 1, c = 0; c < b; c++) {
            ...
            this.setLength(a);
            for (let d = this.buffer.length / a, e = ~~(d / 10) || 1, f = this.buffer.numberOfChannels, g = 0; g < f; g++) {
                for (let h = this.splitPeaks[g], i = this.buffer.getChannelData(g), j = b; j <= c; j++) {
                    for (var k = ~~(j * d), l = ~~(k + d), m = 0, n = 0, o = k; o < l; o += e) {
                        let p = i[o];
                        p > n && (n = p), p < m && (m = p);
                    }
                    h[2 * j] = n, h[2 * j + 1] = m, (g == 0 || n > this.mergedPeaks[2 * j]) && (this.mergedPeaks[2 * j] = n), (g == 0 || m < this.mergedPeaks[2 * j + 1]) && (this.mergedPeaks[2 * j + 1] = m);
                }
            }
            ...
            if (this.buffer) {
                return this.scheduledPause = null, a == null && (a = this.getCurrentTime(), a >= this.getDuration() && (a = 0)), b == null && (b = this.getDuration()), this.startPosition = a, this.lastPlay = this.ac.currentTime, this.state === this.states[this.FINISHED_STATE] && this.setState(this.PAUSED_STATE), {
                    start: a,
                    end: b,
                };
            }
            ...
            if (this.buffer) {
                this.createSource();
                let c = this.seekTo(a, b);
                a = c.start, b = c.end, this.scheduledPause = b, this.source.start(0, a, b - a), this.ac.state == "suspended" && this.ac.resume && this.ac.resume(), this.setState(this.PLAYING_STATE), this.fireEvent("play");
            }
        }, pause: function() {
        ...
        init: function(a) {
            ...
            var a = this, b = function() {
                if (!a.isPaused()) {
                    ...
                    let c = window.requestAnimationFrame || window.webkitRequestAnimationFrame;
                    c(b);
                }
            };
            this.on("play", b);
        }, load: function(a, b, c, d) {
            let e = document.createElement(this.mediaType);
            e.controls = this.params.mediaControls, e.autoplay = this.params.autoplay || !1, e.preload = d == null ? "auto" : d, e.src = a, e.style.width = "100%";
            let f = b.querySelector(this.mediaType);
            ...
            let c = this;
            typeof a.load === "function" && a.load(), a.addEventListener("error", function() {
            ...
            let a = (this.buffer || this.media).duration;
            ...
            a != null && (this.media.currentTime = a), this.clearPlayEnd();
        }, play: function(a, b) {
        ...
        init: function(a, b) {
            ...
            !b && a.preventDefault();
            let c, d = a.targetTouches ? a.targetTouches[0].clientX : a.clientX,
                e = this.wrapper.getBoundingClientRect(), f = this.width, g = this.getWidth();
            ...
            this.wrapper.addEventListener("click", function(b) {
                let c = a.wrapper.offsetHeight - a.wrapper.clientHeight;
                if (c != 0) {
                    let d = a.wrapper.getBoundingClientRect();
            ...
            let b = this.wrapper.scrollWidth * a;
            this.recenterOnPosition(b, !0);
        }, recenterOnPosition: function(a, b) {
            let c = this.wrapper.scrollLeft, d = ~~(this.wrapper.clientWidth / 2), e = a - d, f = e - c,
        ...
        createElements: function() {
            let a = this.wrapper.appendChild(this.style(document.createElement("canvas"), {
                position: "absolute",
                zIndex: 1,
                left: 0,
                top: 0,
            ...
            if (this.waveCc = a.getContext("2d"), this.progressWave = this.wrapper.appendChild(this.style(document.createElement("wave"), {
                ...
                let b = this.progressWave.appendChild(document.createElement("canvas"));
                this.progressCc = b.getContext("2d");
            }
        }, updateSize: function() {
            let a = Math.round(this.width / this.params.pixelRatio);
            ...
            if (b[0] instanceof Array) {
                ...
                if (this.params.splitChannels) {
                    return this.setHeight(g.length * this.params.height * this.params.pixelRatio), void g.forEach(function(a, b) {
                        f.drawBars(a, b, d, e);
                    });
                }
                b = g[0];
            }
            let h = [].some.call(b, function(a) {
                    return a < 0;
                }), i = 1;
            h && (i = 2);
            let j = 0.5 / this.params.pixelRatio, k = this.width, l = this.params.height * this.params.pixelRatio,
                m = l * c || 0, n = l / 2, o = b.length / i, p = this.params.barWidth * this.params.pixelRatio,
                q = Math.max(this.params.pixelRatio, ~~(p / 2)), r = p + q, s = 1 / this.params.barHeight;
            if (this.params.normalize) {
                let t = a.util.max(b), u = a.util.min(b);
                s = -u > t ? -u : t;
            }
            let v = o / k;
            this.waveCc.fillStyle = this.params.waveColor, this.progressCc && (this.progressCc.fillStyle = this.params.progressColor), [this.waveCc, this.progressCc].forEach(function(a) {
                if (a) {
                    for (let c = d / v; c < e / v; c += r) {
                        let f = b[Math.floor(c * v * i)] || 0, g = Math.round(f / s * n);
            ...
            if (!h) {
                for (var i = [], j = 0, k = b.length; j < k; j++) {
                    i[2 * j] = b[j], i[2 * j + 1] = -b[j];
                }
                b = i;
            }
            let l = 0.5 / this.params.pixelRatio, m = this.params.height * this.params.pixelRatio, n = m * c || 0,
                o = m / 2, p = ~~(b.length / 2), q = 1;
            this.params.fillParent && this.width != p && (q = this.width / p);
            let r = 1 / this.params.barHeight;
            if (this.params.normalize) {
                let s = a.util.max(b), t = a.util.min(b);
                r = -t > s ? -t : s;
            }
            this.waveCc.fillStyle = this.params.waveColor, this.progressCc && (this.progressCc.fillStyle = this.params.progressColor), [this.waveCc, this.progressCc].forEach(function(a) {
                if (a) {
        ...
        initDrawer: function(a) {
            if (this.maxCanvasWidth = a.maxCanvasWidth != null ? a.maxCanvasWidth : 4e3, this.maxCanvasElementWidth = Math.round(this.maxCanvasWidth / this.params.pixelRatio), this.maxCanvasWidth <= 1) {
            ...
            for (var a = Math.round(this.width / this.params.pixelRatio), b = Math.ceil(a / this.maxCanvasElementWidth); this.canvases.length < b;) {
                this.addCanvas();
            }
            for (; this.canvases.length > b;) {
                this.removeCanvas();
            }
            for (let c in this.canvases) {
                let d = this.maxCanvasWidth + 2 * Math.ceil(this.params.pixelRatio / 2);
            ...
            let a = {}, b = this.maxCanvasElementWidth * this.canvases.length;
            ...
            let d = Math.round(b / this.params.pixelRatio), e = Math.round(this.width / this.params.pixelRatio);
            ...
            for (let a in this.canvases) {
                this.clearWaveForEntry(this.canvases[a]);
            }
            ...
            h && (i = 2);
            let j = this.width, k = this.params.height * this.params.pixelRatio, l = k * c || 0, m = k / 2,
                n = b.length / i, o = this.params.barWidth * this.params.pixelRatio,
                p = Math.max(this.params.pixelRatio, ~~(o / 2)), q = o + p, r = 1 / this.params.barHeight;
            if (this.params.normalize) {
                let s = a.util.max(b), t = a.util.min(b);
                r = -t > s ? -t : s;
            }
            for (let u = n / j, v = d / u; v < e / u; v += q) {
                let w = b[Math.floor(v * u * i)] || 0, x = Math.round(w / r * m);
            ...
            let l = this.params.height * this.params.pixelRatio, m = l * c || 0, n = l / 2,
                o = 1 / this.params.barHeight;
            if (this.params.normalize) {
                let p = a.util.max(b), q = a.util.min(b);
                o = -q > p ? -q : p;
            }
            this.drawLine(b, o, n, m, d, e), this.fillRect(0, n + m - this.halfPixel, this.width, this.halfPixel);
        }, drawLine: function(a, b, c, d, e, f) {
            for (let g in this.canvases) {
                let h = this.canvases[g];
            ...
            if (b) {
                let i = c.length / 2, j = 1;
                this.params.fillParent && this.width != i && (j = this.width / i);
                let k = Math.round(i * a.start), l = Math.round(i * a.end);
                if (!(k > h || l < g)) {
                    let m = Math.max(k, g), n = Math.min(l, h);
            ...
            for (let e = Math.floor(a / this.maxCanvasWidth), f = Math.min(Math.ceil((a + c) / this.maxCanvasWidth) + 1, this.canvases.length), g = e; g < f; g++) {
                let h = this.canvases[g], i = g * this.maxCanvasWidth, j = {
        ...
        initDrawer: function(a) {
            let b = this;
            for (let c in this.defaultPlotParams) {
                void 0 === this.params[c] && (this.params[c] = this.defaultPlotParams[c]);
            }
        ...
        drawPeaks: function(a, b, c, d) {
            if (this.plotArrayLoaded == 1) {
                this.setWidth(b), this.splitChannels = !0, this.params.height = this.params.height / 2, a[0] instanceof Array && (a = a[0]), this.params.barWidth ? this.drawBars(a, 1, c, d) : this.drawWave(a, 1, c, d), this.params.height = 2 * this.params.height, this.calculatePlots(), this.drawPlots();
            } else {
                let e = this;
        ...
        displayPlotRange: function() {
            let a = this.params.plotRangeFontSize * this.params.pixelRatio,
        ...
        calculatePlots: function() {
            ...
            this.params.plotNormalizeTo == "whole" ? (this.plotMin = d, this.plotMax = c) : this.params.plotNormalizeTo == "values" ? (this.plotMin = this.params.plotMin, this.plotMax = this.params.plotMax) : (this.plotMin = f, this.plotMax = e), this.normalizeValues();
        },
        normalizeValues: function() {
            let a = {};
            if (this.params.plotNormalizeTo !== "none") {
                for (let b in this.plotPoints) {
                    let c = (this.plotPoints[b] - this.plotMin) / (this.plotMax - this.plotMin);
        ...
        loadPlotArrayFromFile: function(b, c, d) {
            void 0 === d && (d = "\t");
            let e = [], f = { url: b, responseType: "text" }, g = a.util.ajax(f);
            g.on("load", function(a) {
                if (a.currentTarget.status == 200) {
                    for (let b = a.currentTarget.responseText.split("\n"), f = 0; f < b.length; f++) {
                        let g = b[f].split(d);
                        g.length == 2 && e.push({ time: parseFloat(g[0]), value: parseFloat(g[1]) });
                    }
                    c(e);
                }
            });
        ...
        init: function() {
            ...
            for (var d = [], e = 0; e < this.peakCacheRanges.length && this.peakCacheRanges[e] < b;) {
                e++;
            }
            ...
            e % 2 == 0 && d.push(c), d = d.filter(function(a, b, c) {
                return b == 0 ? a != c[b + 1] : b == c.length - 1 ? a != c[b - 1] : a != c[b - 1] && a != c[b + 1];
        ...
        let b = function() {
            let b = document.querySelectorAll("wavesurfer");
            Array.prototype.forEach.call(b, function(b) {
                let c = a.util.extend({ container: b, backend: "MediaElement", mediaControls: !0 }, b.dataset);
                b.style.display = "block";
                let d = a.create(c);
                if (b.dataset.peaks) {
                    var e = JSON.parse(b.dataset.peaks);
                }
