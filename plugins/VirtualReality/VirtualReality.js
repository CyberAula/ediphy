...
export function VirtualReality(base) {
    return {
        ...
        parseRichMarkInput: function(x, y, width, height, toolbarState, boxSelected) {
            let xPix = (x - width / 2);
            let yPix = -(y - height / 2);

            // const RATIO = 70;
            const R = 4;
            // let xMet = xPix / width * 10;
            // let yMet = yPix / height * 5;

            let xMet = -xPix / width * 1.75 * Math.PI / 3;
            let yMet = yPix / height * 2 * Math.PI / 3 * height / width;
            let vrApp = document.querySelector('#box-' + boxSelected + ' .VR');
