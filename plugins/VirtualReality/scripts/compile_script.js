...
function buildScript(root, buildDir, input, output) {
    ...
    const cliLocation = path.resolve('node_modules', 'react-native', 'local-cli', 'cli.js');
...
new Promise((resolve, reject) => {
    ...
    fs.mkdir(path.join(projectDir, 'build'), err => {
        if (err) {
            console.log('Failed to create ' + buildDir + ' directory');
            return reject(1);
        }
        resolve();
    });
    ...
    .then(() => {
        return Promise.all([
            buildScript(
                ...
                path.resolve(projectDir, 'build-custom', 'index.bundle.js'),
            ),
            buildScript(
                projectDir,
                buildDir,
                path.resolve(projectDir, 'client.js'),
                path.resolve(projectDir, 'build-custom', 'client.bundle.js'),
            ),
        ]);
    })
    .catch(err => {
