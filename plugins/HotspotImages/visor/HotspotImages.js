    ...
    return {
        getRenderTemplate: function (state,id) {
            ...
            let markElements = marks.map((e) =>{
               return(
                   <a key={e.key} style={{position: 'absolute', top :e.props.style.top,left: e.props.style.left}} href="#" onClick={this.onMarkClicked(box_id, e.props.style.top + "," + e.props.style.left)}><i style={{width:"100%",height:"100%"}} className="material-icons">room</i></a>
               );
            });
